...
def load_dbase():
    ...
    # read_data_join['Opened'] = pd.to_datetime(read_data_join['Opened'], errors='ignore')  # FIRST CONVERSION
    # read_data_join['Opened'] = read_data_join['Opened'].dt.strftime('%Y-%m-%d')  # REMOVE TIME INFORMATION
    # read_data_join['Opened'] = pd.to_datetime(read_data_join['Opened'], errors='ignore')  # SECOND CONVERSION
    #
    # # COLUMN Cust IA DATE TIME FORMATTER
    # read_data_join['Cust IA'] = pd.to_datetime(read_data_join['Cust IA'], errors='ignore')  # JUST CONVERSION
    #
    # # RENAME COLUMNS
...
if __name__ == '__main__':
    ...
    ActionTaskOne.company_work_join_file_maker()                # ORDER EXECUTION 001A
    ActionTaskOne.company_colet_day_report_maker()              # ORDER EXECUTION 001
    ActionTaskOne.filter_task_red_phrases()                     # ORDER EXECUTION 002 -> RED
    ActionTaskOne.filter_task_scheduled_phrases()               # ORDER EXECUTION 003 -> BLUE
    ActionTaskOne.filter_task_no_red_phrases()                  # ORDER EXECUTION 004 -> YELLOW
    ...
    ActionTaskOne.count_live_day_task()
